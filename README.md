...
special importance to privacy-oriented users:
 ...
 - do not support Dinastycoin testnet nor stagenet networks,
...
The key features of the Dinastycoin Blockchain Explorer are:
 ...
 - showing public components of Dinastycoin addresses,
 - decoding which outputs and mixins belong to the given Dinastycoin address and viewkey,
 - can prove that you send Dinastycoin to someone,
 - detailed information about ring members, such as, their age, timescale and their ring sizes,
 - showing number of amount output indices,
 - support Dinastycoin testnet and stagnet networks,
...
Once the Dinastycoin compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/dinastycoin
cd ~
...
By default it will look for blockchain in its default location i.e., `~/.dinastycoin/lmdb`.
You can use `-b` option if its in different location.

For example:

```bash
./dcyblocks -b /home/ubuntu/non-defult-dinastycoin-location/lmdb/
```
...
dcyblocks, Dinastycoin Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Dinastycoin total emission monitoring
                                        thread
...
Obtaining current Dinastycoin emission amount is not straight forward. Thus, by default it is
disabled. To enable it use `--enable-emission-monitor` flag, e.g.,


```bash
dcyblocks --enable-emission-monitor
```
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.dinastycoin/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.dinastycoin/testnet/lmdb/emission_amount.txt` or `~/.dinastycoin/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.dinastycoin/lmdb/emission_amount.txt`
...
```
Dinastycoin emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Dinastycoin daemon's  `print_coinbase_tx_sum` command.
...
Having the `crt` and `key` files, run `dcyblocks` in the following way:

```bash
./dcyblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key
```
...
```bash
# we use here official Dinastycoin project's donation address as an example
...
We use recipient's address (i.e. not our address from which we sent dcy to recipient).
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_dinastycoin",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "dinastycoin_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```

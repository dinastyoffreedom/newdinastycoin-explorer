...
// forked version of the rx_slow_hash from dinastycoin
...
// from dinastycoin to use me_rx_slow_hash
bool
...
struct tx_details
{
    ...
    // public keys and dcy amount of outputs
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_dinastycoin_network_info(j_info);
...
show_ringmemberstx_jsonhex(string const& tx_hash_str)
{
    ...
    // private dinastycoin networks, so we can easly put these
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Dinastycoin address not provided!");
    }
    ...
    // parse string representing given dinastycoin address
    ...
    if (!xmreg::parse_str_address(xmr_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse dcy address: " + xmr_address_str);
    }
...
//        }
    ...
    for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some dcy.
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
    ...
    // to hold sum of dcy in matched mixins, those that
    ...
    // we get number of all posibilities, and their total dcy amount
...
//                                << endl;
    ...
    // useful for unit testing as it provides total dcy sum
    // of possible mixins
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    else
    {
        ...
        if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
        {
            ...
            // used in mydinastycoin and opendinastycoin projects.
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            // provide total mount of inputs dcy
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_dinastycoin_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    // get dcy address stored in this key image file
...
search(string search_text)
{
    ...
    // check if dinastycoin address is given based on its length
    ...
    // check if integrated dinastycoin address is given based on its length
...
json_rawtransaction(string tx_hash_str)
{
    ...
    // get raw tx json as in dinastycoin
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Dinastycoin address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse dinastycoin address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_dinastycoin_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get dinastycoin network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"dinastycoin_version_full" , string {DINASTYCOIN_VERSION_FULL}},
               ...
               uint64_t bc_height = 0)
{
    ...
    // https://github.com/dinastycoin-project/dinastycoin/pull/1358/commits/7abfc5474c0f86e16c405f154570310468b635c2
    txd.pk = xmreg::get_tx_pub_key_from_received_outs(tx);
    txd.additional_pks = cryptonote::get_additional_tx_pub_keys_from_extra(tx);


    // sum dcy in inputs and ouputs in the given tx
...
bool
get_dinastycoin_network_info(json& j_info)
{
